...
public class LoginController {
	...
	public String userdashboard(@ModelAttribute("login") Registration registration,Model model,HttpServletRequest request,RedirectAttributes redirectAttributes)
		...
		if(reg.getId()!=null) 
		{
			...
			{
				if(reg.getRole().equals(AppConstants.KEY_FOR_USER_ROLL_NAME) && reg.getLockstatus().equals("unlocked")) 
				{
				...
				if(reg.getRole().equals("agency") && reg.getActivestatus().equals("Y") && reg.getLockstatus().equals("unlocked"))
				{
				return AppConstants.LOG_VIEW_FOR_AGENCY_DASHBOARD;
				}
			}	
		}
		else 
